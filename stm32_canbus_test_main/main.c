...
#define CAN_BOARD_NAME FC
#define CAN_DATA_TYPE_1 Gas
#define CAN_DATA_TYPE_2 Brake
#define CAN_DATA_TYPE_3 FC_Pressure
#define CAN_BOARD_FILTER_1 UI
/* USER CODE END PD */
...
int main(void) {
	...
	can1 = CAN_new();
	can1.init(&can1, CAN_BOARD_NAME, &hcan1);
	can1.addFilterDevice(&can1, CAN_BOARD_FILTER_1);

	can1.addFilterDeviceData(&can1, DAQ_Board, Longitude); //Add all filters before running the begin function
	can1.addFilterDevice(&can1, DAQ_Board);
