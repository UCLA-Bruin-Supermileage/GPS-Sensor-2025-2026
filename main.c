...
struct Location translateGNRMC(unsigned char* NMEA_sentence) {
...
int main(void)
{
  ...
  struct Location GPSlocation;
  /* Infinite loop */
  /* USER CODE BEGIN WHILE */
  while (1)
  {
    /* USER CODE END WHILE */
	  HAL_StatusTypeDef status = HAL_I2C_Master_Receive(&hi2c1, slave_address, buffer, sizeof(buffer), 250);

	  if (status == HAL_OK) {
		  // now process our char buffer
